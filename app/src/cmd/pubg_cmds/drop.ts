...
export class Drop extends Command {
    ...
    async run(bot: DiscordClientWrapper, msg: Discord.Message, params: string[], perms: number) {
        ...
        const explanation: string = `**${msg.author.username}**, use the **E**, **M**, **S**, and **V** **reactions** to switch between **Erangel**, **Miramar**, **Sanhok**, and **Vikendi**.`;
        if (map) {
    ...
    private getMapImage(map: string): PubgMapImageLocation {
        switch (map) {
            ...
            case 'v': {
                return PubgMapImageLocation.VIKENDI;
            }
        }
    }
    ...
    private async setupReactions(msg: Discord.Message, originalPoster: Discord.User): Promise<void> {
        ...
        const v_filter: Discord.CollectorFilter = (reaction, user) => reaction.emoji.name === reaction_numbers[3] && originalPoster.id === user.id;
        ...
        const v_collector: Discord.ReactionCollector = msg.createReactionCollector(v_filter, { time: 15 * 1000 });

        const explanation: string = `**${originalPoster.username}**, use the **E**, **M**, **S**, and **V** **reactions** to switch between **Erangel**, **Miramar**, **Sanhok**, and **Vikendi**.`;
        ...
        v_collector.on('collect', async () => {
            if (msg.deletable) {
                await msg.delete().catch(() => {});
            }

            let reply: Discord.Message = (await msg.channel.send('Finding a drop ...')) as Discord.Message;
            const drop: DropLocation = this.getDrop('v');
            const attatchment: Discord.Attachment = await this.createImage('v', drop);
    ...
    private getRandomVikendi(): DropLocation {
        let drops: DropLocation[] = [
            { name: 'Abbey', x: 235, y: 340 },
            { name: 'Castle', x: 291, y: 294 },
            { name: 'Cantra', x: 361, y: 362 },
            { name: 'Cement Factory', x: 357, y: 266 },
            { name: 'Coal Mine', x: 248, y: 153 },
            { name: 'Cosmodrome', x: 310, y: 122 },
            { name: 'Dino Park', x: 185, y: 312 },
            { name: 'Dobro Mesto', x: 88, y: 205 },
            { name: 'Goroka', x: 205, y: 195 },
            { name: 'Hot Springs', x: 380, y: 346 },
            { name: 'Lumber Yard', x: 408, y: 286 },
            { name: 'Krichas', x: 142, y: 149 },
            { name: 'Milnar', x: 175, y: 400 },
            { name: 'Mount Kreznic', x: 268, y: 196 },
            { name: 'Movatra', x: 135, y: 303 },
            { name: 'Peshkova', x: 398, y: 183 },
            { name: 'Pilnec', x: 245, y: 400 },
            { name: 'Podvosto', x: 319, y: 227 },
            { name: 'Port', x: 190, y: 90 },
            { name: 'Sawmill', x: 154, y: 340 },
            { name: 'Tovar', x: 248, y: 290 },
            { name: 'Trevno', x: 388, y: 102 },
            { name: 'Vihar', x: 107, y: 301 },
            { name: 'Villa', x: 230, y: 250 },
            { name: 'Volnova', x: 303, y: 353 },
            { name: 'Winery', x: 267, y: 397 },
            { name: 'Zabava', x: 230, y: 100 }
        ];
